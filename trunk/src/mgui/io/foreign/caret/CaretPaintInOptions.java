...
public class CaretPaintInOptions extends SurfaceDataInputOptions {
	...
	public String name_map_prefix;
	
	public void setFiles(File[] files) {
		this.files = files;
		ArrayList<String> f_names = new ArrayList<String>();
		CaretPaintLoader loader = new CaretPaintLoader();
		...
		for (int i = 0; i < files.length; i++){
			...
			if (header != null){
				for (int j = 0; j < header.columns.items.size(); j++)
					f_names.add(((String)header.columns.items.get(j).objValue));
				columns[i] = header.columns.getSize();
				}
